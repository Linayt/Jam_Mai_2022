...
[CustomEditor(typeof(EnableGameObjectsOnInput))]
public class EnableGameObjectsOnInputEditor : Editor
{
	...
	[Tooltip("Used to check public variables from the target class")]
	private EnableGameObjectsOnInput myObject;
	...
	{
		myObject = (EnableGameObjectsOnInput) target;
				...
				case "Input":
				{
					...
					{						
						...
						EditorGUILayout.BeginVertical(UIHelper.SubStyle2);
						{
							...
							if (myObject.revertOn == EnableGameObjectsOnInput.RevertOn.Timer)
							{
				...
				case "GameObjects":
				{
					EditorGUILayout.BeginVertical(UIHelper.SubStyle1);
					{
						EditorGUILayout.BeginHorizontal();
						{
							...
							if (GUILayout.Button(" Add GameObjects ", UIHelper.GreenButtonStyle,
