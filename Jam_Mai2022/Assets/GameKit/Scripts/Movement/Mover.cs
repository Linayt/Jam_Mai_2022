...
public class Mover : MonoBehaviour
{
	...
	private void HandleParticleEmission()
	{
		...
		var hvMagnitude = movementType == MovementType.Rigidbody
	...
	private void GetSpeed ()
	{
		...
		currentSpeed.x = useXAxis ? (requiresInput ? Input.GetAxis(xInputName) * xSpeed : xSpeed) : currentSpeed.x;
		currentSpeed.y = useYAxis ? (requiresInput ? Input.GetAxis(yInputName) * ySpeed : ySpeed) : currentSpeed.y;
		currentSpeed.z = useZAxis ? (requiresInput ? Input.GetAxis(zInputName) * zSpeed : zSpeed) : currentSpeed.z;
	}
	...
	{
		//Debug.Log("Current speed y : " + currentSpeed.y);
